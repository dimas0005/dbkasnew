...
$data = mysqli_query($con,"SELECT * FROM pengeluaran ORDER BY tanggal DESC");

$totalQ = mysqli_query($con,"
    SELECT SUM(total) AS total FROM pengeluaran
");
...
<a href="pengeluaran.php" style="display:block; padding:12px 20px; color:white; text-decoration:none; background:rgba(0,0,0,0.2);">Pengeluaran</a>
...
<h2>Kas Pengeluaran</h2>

<div style="background:#fff; padding:20px; border-radius:10px; width:400px;">

<h3><?= isset($edit) ? "Edit Pengeluaran" : "Tambah Pengeluaran" ?></h3>
